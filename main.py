...
if __name__ == "__main__":   
    ...
    while True:
        ...
        if stock_data is None:
            ...
            if ans == "n": 
                ...
                single_stock_training = True
            else: 
                print("Multiple stocks option chosen.")
                single_stock_training = False
            break
        
    # Single-stock processing
    if single_stock_training:
        ...
        # Sets training_time to a point in time
    ...
    # Multi-stock processing
        ...
        # Append the user stock to the training stocks list
        training_stocks.append(stock)
        ...
        for t_stock in training_stocks:
            ...
            lookback = 3
            t_shifted_data = prepare_dataframe_for_lstm(t_data, lookback)    
            
            # Scale the data for the LSTM model
            scaled_data = scaler.fit_transform(t_shifted_data)    
            ...
            minutes = int(elapsed_time/60)
            seconds = int(elapsed_time%60)
            print(f"Training Time for {t_stock}: {minutes}:{seconds}")
            print()

        # Final test on the model
        print("Final Test for " + t_stock + ":")
        test(model, test_loader, criterion)
